...
public:
  ...
  PeptideMatch(const string& a, const fstream::pos_type p, const string& s, const double sc, ExpPeptide* ep, const unsigned f, const unsigned l){
    ac = a;
    pos = p;
    score = sc;
    pValue = 1.0;
  ...
  for (unsigned frag = 0; frag < theoSpectrum.size(); frag++){
	...
	if ((theoMoz >= minMass) && (theoMoz <= maxMass)){
	  ...
	  if (left-right == 2)
	    ...
	    // Normal case
	    closest = (ep->fragments[left].moz-theoMoz < theoMoz-ep->fragments[right].moz) ? left : right;
...
double computeSPCScore(const vector<vector<int> >& spMatch, const vector<TheoreticalFragmentation>& theoSpectrum, ExpPeptide* ep, const string& peptSeq)
{
  double score = 0.0;
...
void matchMSMSSpectra(const string& peptSeq, ExpPeptide* ep, const string& scoring, vector<PeptideMatch>& peptideMatches, const string& ac, const fstream::pos_type pos, unsigned first, unsigned last)
{
...
void searchFastaFile(vector<PeptideMatch>& peptideMatches, const string fasta, unsigned nmc, const vector<Mass2Pept>& massIndex, const string& scoring)
{
  ...
  while (fastaFile.good()){
	...
	for (unsigned j = closest; (j >= 0) && (massIndex[j].mass >= theoMass*(1.0-tol)); j--)
	  matchMSMSSpectra(peptSeq, massIndex[j].expPeptide, scoring, peptideMatches, ac, pos, pept[i].first, pept[i].last);
	for (unsigned j = closest+1; (j < massIndex.size()) && (massIndex[j].mass <= theoMass*(1.0+tol)); j++)
	  matchMSMSSpectra(peptSeq, massIndex[j].expPeptide, scoring, peptideMatches, ac, pos, pept[i].first, pept[i].last);
      }
    }
...
void groupPeptideMatches(vector<ProteinMatch>& protMatches, const vector<PeptideMatch>& peptideMatches, const string& fasta)
{
  ProteinMatch pm;
  ...
  for (unsigned i = 0; i < protMatches.size(); i++){
    ...
    for (unsigned j = 0; j < protMatches[i].peptMatch.size(); j++){
      // Flags matched parts
      for (unsigned k = peptideMatches[protMatches[i].peptMatch[j]].first; k <= peptideMatches[protMatches[i].peptMatch[j]].last; k++)
	covered[k] = true;
	...
	already[peptideMatches[protMatches[i].peptMatch[j]].sequence] = peptideMatches[protMatches[i].peptMatch[j]].score;
      else if (already[peptideMatches[protMatches[i].peptMatch[j]].sequence] < peptideMatches[protMatches[i].peptMatch[j]].score)
	already[peptideMatches[protMatches[i].peptMatch[j]].sequence] = peptideMatches[protMatches[i].peptMatch[j]].score;
    }
...
void outputProteinMatches(const vector<ProteinMatch>& protMatches, const vector<PeptideMatch>& peptideMatches)
{
  ...
  for (int i = protMatches.size()-1; i >= 0; i--){
    cout<<protMatches[i].score<<"  "<<protMatches[i].ac<<" "<<protMatches[i].id<<" "<<protMatches[i].descr<<endl;
    ...
    for (int j = protMatches[i].peptMatch.size()-1; j >= 0; j--)
      cout<<"    "<<peptideMatches[protMatches[i].peptMatch[j]].sequence<<" "<<peptideMatches[protMatches[i].peptMatch[j]].score<<" spectrum="<<peptideMatches[protMatches[i].peptMatch[j]].expPeptide->order<<endl;
    cout<<endl;
  }
